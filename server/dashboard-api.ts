...
function findCapacityViolation(requested: Map<string, number>, committed: Map<string, number>, products: Product[]): CapacityViolation | undefined {
  ...
  requested.forEach((quantity, productId) => {
    ...
    if (total > product.stretchCapacity) {
      violation = { productId, productName: product.name, committed: committedQuantity, requested: quantity, maxCapacity: product.stretchCapacity, standardCapacity: product.standardCapacity };
    }
  });
  return violation;
}
...
export function registerDashboardApi(app: Express) {
  ...
  app.post("/api/dashboard/enquiries", async (req, res) => {
    ...
    const enquiry: Enquiry = {
      id: nanoid(12), enquiryNumber: `ENQ-${Date.now().toString().slice(-6)}`, createdAt: now, updatedAt: now,
      productName: body.productName.trim(), productSlug: body.productSlug.trim(), selectedDay, quantity: body.quantity.trim(), name: body.name.trim(), mobileNumber: body.mobileNumber.trim(), deliveryAddress: body.deliveryAddress.trim(), status: "Enquiry Received", source: "WhatsApp",
    };
  ...
  app.delete("/api/dashboard/enquiries/:id", async (req, res) => {
    const state = await readEnquiries();
    const index = state.enquiries.findIndex((item) => item.id === req.params.id);
