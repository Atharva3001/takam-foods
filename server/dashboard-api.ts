...
function findCapacityViolation(requested: Map<string, number>, committed: Map<string, number>, products: Product[]): CapacityViolation | undefined {
  ...
  requested.forEach((quantity, productId) => {
    ...
    if (total > product.stretchCapacity) violation = { productId, productName: product.name, committed: committedQuantity, requested: quantity, maxCapacity: product.stretchCapacity, standardCapacity: product.standardCapacity };
  });
  return violation;
}
...
function ensureScheduledProducts(state: Awaited<ReturnType<typeof readDashboard>>, date: string) {
  ...
  const products = schedule.products.map((spec) => {
    ...
    if (!product) {
      ...
      product.unit = spec.unit; product.standardCapacity = spec.standardCapacity; product.stretchCapacity = spec.maxCapacity;
    }
    return product;
  });
...
function parseEnquiryQuantity(quantity: string, product: Product) {
  ...
  if (product.unit === "kg" && lower.includes("gm")) return value / 1000;
  return value;
}
...
export function registerDashboardApi(app: Express) {
  ...
  app.put("/api/dashboard", async (req, res) => {
    ...
    await writeDashboard({ products: body.products, orders: body.orders }); res.json({ ok: true });
  });
  ...
  app.patch("/api/dashboard/enquiries/:id", async (req, res) => {
    const enquiryState = await readEnquiries(); const enquiry = enquiryState.enquiries.find((item) => item.id === req.params.id);
    if (!enquiry) return sendError(res, "Enquiry not found", 404);
    const nextStatus = req.body.status as EnquiryStatus | undefined;
    if (nextStatus && !enquiryStatuses.has(nextStatus)) return sendError(res, "Invalid enquiry status");
    ...
    if (nextStatus === "Confirmed" && !enquiry.convertedOrderId) {
      ...
      const product = products.find((item) => item.name.trim().toLowerCase() === enquiry.productName.trim().toLowerCase());
      ...
      const requested = new Map<string, number>([[product.id, quantity]]);
      const committed = new Map<string, number>();
      for (const order of dashboardState.orders) {
        if (order.productionDate !== enquiry.selectedDay || capacityReleasingStatuses.has(order.status)) continue;
      ...
      const order: Order = {
        id: nanoid(12), orderNumber: `TF-${Date.now().toString().slice(-6)}`, orderDate: now.slice(0, 10), productionDate: enquiry.selectedDay,
        customerName: enquiry.name, customerPhone: enquiry.mobileNumber, deliveryAddress: enquiry.deliveryAddress,
        items: [{ productId: product.id, quantity }], amount: 0, paymentStatus: "Pending", status: "Confirmed", deliveryPerson: "", trackingNumber: "", actualDeliveryDate: "", notes: `Converted from ${enquiry.enquiryNumber}`, createdAt: now, updatedAt: now,
      };
      dashboardState.orders.push(order);
      await writeDashboard(dashboardState);
      enquiry.convertedOrderId = order.id; enquiry.convertedOrderNumber = order.orderNumber;
    }

    Object.assign(enquiry, req.body, { updatedAt: new Date().toISOString() });
    await writeEnquiries(enquiryState);
    res.json(enquiry);
  });
  app.delete("/api/dashboard/enquiries/:id", async (req, res) => {
    const state = await readEnquiries(); const index = state.enquiries.findIndex((item) => item.id === req.params.id);
    if (index === -1) return sendError(res, "Enquiry not found", 404);
    state.enquiries.splice(index, 1); await writeEnquiries(state); res.json({ ok: true });
  });
  ...
  app.post("/api/dashboard/products", async (req, res) => {
    ...
    const state = await readDashboard(); const product: Product = { id: nanoid(10), name: name.trim(), unit, standardCapacity: Number(standardCapacity), stretchCapacity: Math.max(Number(stretchCapacity), Number(standardCapacity)), active: true };
    state.products.push(product); await writeDashboard(state); res.status(201).json(product);
  });
  app.patch("/api/dashboard/products/:id", async (req, res) => {
    const state = await readDashboard(); const product = state.products.find((item) => item.id === req.params.id);
    if (!product) return sendError(res, "Product not found", 404);
    Object.assign(product, req.body); product.standardCapacity = Number(product.standardCapacity); product.stretchCapacity = Math.max(Number(product.stretchCapacity), product.standardCapacity);
    await writeDashboard(state); res.json(product);
  });
  app.post("/api/dashboard/orders", async (req, res) => {
    ...
    const state = await readDashboard(); const { schedule, products } = ensureScheduledProducts(state, body.productionDate);
    if (!schedule) return sendError(res, "No production timetable is configured for this date", 409);
    const requested = new Map<string, number>();
    for (const item of body.items) {
      const quantity = Number(item.quantity); const product = products.find((p) => p.id === item.productId);
    ...
    const capacityViolation = findCapacityViolation(requested, committed, products);
    if (capacityViolation) return res.status(409).json({ error: `${capacityViolation.productName} is full for ${body.productionDate}`, ...capacityViolation });
    const now = new Date().toISOString();
    const order: Order = { id: nanoid(12), orderNumber: body.orderNumber || `TF-${Date.now().toString().slice(-6)}`, orderDate: body.orderDate || now.slice(0, 10), productionDate: body.productionDate, customerName: body.customerName.trim(), customerPhone: body.customerPhone || "", deliveryAddress: body.deliveryAddress || "", items: body.items.map((item) => ({ productId: item.productId, quantity: Number(item.quantity) })), amount: Number(body.amount || 0), paymentStatus: body.paymentStatus || "Pending", status: body.status || "New", deliveryPerson: body.deliveryPerson || "", trackingNumber: body.trackingNumber || "", actualDeliveryDate: body.actualDeliveryDate || "", notes: body.notes || "", createdAt: now, updatedAt: now };
    state.orders.push(order); await writeDashboard(state); res.status(201).json(order);
  });
  app.patch("/api/dashboard/orders/:id", async (req, res) => {
    const state = await readDashboard(); const order = state.orders.find((item) => item.id === req.params.id);
    if (!order) return sendError(res, "Order not found", 404); Object.assign(order, req.body, { updatedAt: new Date().toISOString() }); await writeDashboard(state); res.json(order);
  });
  app.get("/api/dashboard/production", async (req, res) => {
    const date = String(req.query.date || new Date().toISOString().slice(0, 10)); const state = await readDashboard(); const { schedule, products } = ensureScheduledProducts(state, date);
  ...
  app.get("/api/dashboard/capacity", async (req, res) => {
    const date = String(req.query.date || new Date().toISOString().slice(0, 10)); const state = await readDashboard(); const { schedule, products } = ensureScheduledProducts(state, date);
    if (!schedule) return res.json([]); const committed = new Map<string, number>();
    for (const order of state.orders) { if (order.productionDate !== date || capacityReleasingStatuses.has(order.status)) continue; for (const item of order.items) committed.set(item.productId, (committed.get(item.productId) || 0) + item.quantity); }
