...
import { ArrowLeft, CheckCircle2, CircleAlert, PackagePlus, Plus, RefreshCw, Settings2, Truck } from "lucide-react";
  ...
  return <main className="min-h-screen bg-background px-4 py-5 sm:px-6 lg:px-8">
    <div className="mx-auto max-w-7xl">
      ...
      <section className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Stat label="Orders" value={totals.orders} tone="bg-peach" />
        <Stat label="Open orders" value={totals.open} tone="bg-mint" />
        <Stat label="Delivered" value={totals.delivered} tone="bg-mascot" />
        <Stat label="Sales" value={`₹${totals.sales.toLocaleString("en-IN")}`} tone="bg-white" />
      </section>
      ...
      <div className="mb-5 flex flex-wrap items-center gap-2 border-b-2 border-ink pb-3">
        ...
        {tab === "capacity" && <label className="ml-auto flex items-center gap-2 text-sm font-extrabold">Production date <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="rounded-lg border-2 border-ink bg-white px-3 py-2 font-bold" /></label>}
      </div>

      {tab === "capacity" && <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {capacity.length === 0 && <Empty title="No products yet" text="Add your modaks and their daily production limits to start using the capacity view." action={() => setShowProduct(true)} />}
