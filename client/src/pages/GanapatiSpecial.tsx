                ...
                {elevenDayPlan.map((day, index) => {
                  ...
                  return (
                    <article key={day.day} className={`sticker bg-white p-4 md:p-5 flex gap-4 md:gap-5 items-center ${index % 2 ? "rotate-[.35deg]" : "-rotate-[.35deg]"}`}>
                      ...
                      <div className="min-w-0 flex-1"><div className="flex flex-wrap justify-between gap-x-4 gap-y-1"><div><h3 className="font-display font-extrabold text-xl md:text-2xl">{day.name}</h3><span className="font-bold text-xs md:text-sm text-tomato">Day {index + 1}</span></div><span className="font-bold text-xs md:text-sm text-muted-foreground">{day.mood}</span></div><div className="mt-3 flex flex-wrap gap-2">{dayProducts.map((product) => { const booked = product.slug === "ukadiche-modak" && (day.date === "14 Sep" || day.date === "15 Sep"); return <Link key={product.slug} href={`/product/${product.slug}`} className={`inline-flex items-center gap-1.5 border-2 border-ink rounded-full pl-1 pr-2.5 py-1 text-xs md:text-sm font-display font-bold hover:bg-mascot ${booked ? "bg-peach" : "bg-mint/50"}`}><span className="h-7 w-7 shrink-0 overflow-hidden rounded-full border border-ink/20 bg-white flex items-center justify-center"><img src={product.images[0]} alt="" aria-hidden className={`h-full w-full ${product.images[0].includes("takam_modak_") ? "object-contain p-0.5" : "object-cover"}`} /></span>{product.marathi}{booked ? " • Fully Booked" : ""}</Link>; })}</div></div>
                    </article>
                  );
                })}
              </div>
